...
public class ExcelAutomation {
	...
	public ExcelAutomation(boolean visible) {
		ComThread.InitSTA();// 线程启动
		if (excel == null) {
			excel = new ActiveXComponent("Excel.Application");
			excel.setProperty("Visible", new Variant(visible)); // 不可见打开excel
	...
	public void createNewWorkbook() {
		...
		selection = Dispatch.get(excel, "Selection").toDispatch();
	}
	 ...
	 * @param docPath
	 */
	public void openWorkbook(String docPath) {
		// closeWorkbook();
		workbook = Dispatch.call(workbooks, "Open", docPath).toDispatch();
		selection = Dispatch.get(excel, "Selection").toDispatch();
	}
	 ...
	 * @param pwd-密码
	 */
	public void openWorkbookOnlyRead(String docPath, String pwd) throws Exception {
		// closeWorkbook();
		workbook = Dispatch.callN(workbooks, "Open", new Object[] { docPath, new Variant(false), new Variant(true),
	 ...
	 * @throws Exception
	 */
	public void openWorkbook(String docPath, String pwd) throws Exception {
		File file=new File(docPath);
		if(file.canWrite()==false){
			workbook = Dispatch
					.callN(workbooks, "Open",
	...
	public boolean find(String toFindText) {
		if (toFindText == null || toFindText.equals(""))
			return false;
		// 从selection所在位置开始查询
		Dispatch find = excel.call(selection, "Find").toDispatch();
		// 设置要查找的内容
		...
		// 全字匹配
		Dispatch.put(find, "MatchWholeExcel", "false");
		// 查找并选中
	...
	public void insertImage(String imageFilePath) {
		Dispatch selection = Dispatch.get(excel, "Selection").toDispatch();
	...
	public void save(String savePath) {
		// Dispatch.call(Dispatch.call(excel, "ExcelBasic").getDispatch(),
		// "FileSaveAs", savePath);
	...
	public void saveAsHtml(String htmlPath) {
		Dispatch.invoke(workbook, "SaveAs", Dispatch.Method, new Object[] { htmlPath, new Variant(8) }, new int[1]);
	}
	 ...
	 */
	public void closeWorkbook(int val) {
		if (workbook != null) {
			Dispatch.call(workbook, "Close", new Variant(val));
			workbook = null;
		}
	}
	...
	public void closeWorkbookWithoutSave() {
		if (workbook != null) {
	...
	public void mergeCell(int tableIndex, int fstCellRowIdx, int fstCellColIdx, int secCellRowIdx, int secCellColIdx) {
		// 所有表格
		Dispatch tables = Dispatch.get(workbook, "Tables").toDispatch();
		// 要填充的表格
	...
	public void copyTableFromAnotherDoc(String anotherDocPath, int tableIndex, String pos) {
		Dispatch doc2 = null;
		try {
			doc2 = Dispatch.call(workbooks, "Open", anotherDocPath).toDispatch();
			// 所有表格
	...
	public void setProp(String sName, String sValue) {
		Dispatch props = Dispatch.get(workbook, "CustomWorkbookProperties").toDispatch();
	...
	public String getProp(String sName) {
		...
		try {
			Dispatch prop0 = Dispatch.call(workbook, "CustomWorkbookProperties", sName).toDispatch();
	...
	public void fackChange() {
		Dispatch selection = Dispatch.call(workbook, "Range", 0, 0).toDispatch();
	...
	/**
	 * 保护当前档,如果不存在, 使用expression.Protect(Type, NoReset, Passexcel)
	 * 
	 * @param pwd
	 * @param type
	 ...
	 * 
	 */
	public void protectedExcel(String pwd, String type) {
		String protectionType = Dispatch.get(workbook, "ProtectionType").toString();
		if (protectionType.equals("-1")) {
			Dispatch.call(workbook, "Protect", Integer.parseInt(type), new Variant(true), pwd);
		}
	}
	 ...
	 * 
	 */
	public void unProtectedExcel(String pwd) {
		String protectionType = Dispatch.get(workbook, "ProtectionType").toString();
		if (!protectionType.equals("0") && !protectionType.equals("-1")) {
			Dispatch.call(workbook, "Unprotect", pwd);
		}
	}
	...
	public String getProtectedType() {
		return Dispatch.get(workbook, "ProtectionType").toString();
	}
	...
	public void setAutomationSecurity(int value) {
		excel.setProperty("AutomationSecurity", new Variant(value));
	}

	/**
	 * 在excel中插入标签 labelName是标签名，labelValue是标签值
	 * 
	 * @param labelName
	 * @param labelValue
	 */
	public void insertLabelValue(String labelName, String labelValue) {
		...
		Dispatch bookMarks = Dispatch.call(workbook, "Bookmarks").toDispatch();
