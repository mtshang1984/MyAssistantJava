...
public class PowerPointAutomation {
	...
	public PowerPointAutomation(boolean visible) {
		ComThread.InitSTA();// 线程启动
		if (powerPoint == null) {
			powerPoint = new ActiveXComponent("PowerPoint.Application");
			powerPoint.setProperty("Visible", new Variant(visible)); // 不可见打开power_point
	...
	public void createNewPresentation() {
		presentation = Dispatch.call(presentations, "Add").toDispatch();
		// selection = Dispatch.get(power_point, "Selection").toDispatch();
	}
	 ...
	 * @param docPath
	 */
	public void openPresentation(String docPath) {
		File file=new File(docPath);
		if(file.renameTo(file)==true){
			presentation = Dispatch.call(presentations, "Open", docPath).toDispatch();
		}
	}
	 ...
	 * @param pwd-密码
	 */
	public void openPresentationOnlyRead(String docPath, String pwd) throws Exception {
		// closePresentation();
		presentation = Dispatch.callN(presentations, "Open", new Object[] { docPath, new Variant(false),
				new Variant(true), new Variant(true), pwd, "", new Variant(false) }).toDispatch();
		selection = Dispatch.get(powerPoint, "Selection").toDispatch();
	}
	 ...
	 * @throws Exception
	 */
	public void openPresentation(String docPath, String pwd) throws Exception {
		// closePresentation();
		presentation = Dispatch
				.callN(presentations, "Open",
	...
	public boolean find(String toFindText) {
		if (toFindText == null || toFindText.equals(""))
			return false;
		// 从selection所在位置开始查询
		Dispatch find = powerPoint.call(selection, "Find").toDispatch();
		// 设置要查找的内容
		...
		// 全字匹配
		Dispatch.put(find, "MatchWholePowerPoint", "false");
		// 查找并选中
	...
	public void insertImage(String imageFilePath) {
		Dispatch selection = Dispatch.get(powerPoint, "Selection").toDispatch();
	...
	public void save(String savePath) {
		// Dispatch.call(Dispatch.call(power_point,
		// "PowerPointBasic").getDispatch(), "FileSaveAs", savePath);
		Dispatch.call(presentation, "SaveAs", savePath);
	}
	...
	public void saveAsHtml(String htmlPath) {
		Dispatch.invoke(presentation, "SaveAs", Dispatch.Method, new Object[] { htmlPath, new Variant(8) }, new int[1]);
	}
	...
	public void closePresentationWithoutSave() {
		if (presentation != null) {
			Dispatch.call(presentation, "Close");
			presentation = null;
		}
	...
	public void close() {
		closePresentation();
		if (powerPoint != null) {
			// Dispatch.call(power_point, "Quit");
			powerPoint.invoke("Quit", new Variant[] {});
			powerPoint = null;
		}
		selection = null;
		presentations = null;
	...
	public void printFile() {
		if (presentation != null) {
			Dispatch.call(presentation, "PrintOut");
		}
	}
	...
	public void mergeCell(int tableIndex, int fstCellRowIdx, int fstCellColIdx, int secCellRowIdx, int secCellColIdx) {
		// 所有表格
		Dispatch tables = Dispatch.get(presentation, "Tables").toDispatch();
		// 要填充的表格
	...
	public void copyTableFromAnotherDoc(String anotherDocPath, int tableIndex, String pos) {
		Dispatch doc2 = null;
		try {
			doc2 = Dispatch.call(presentations, "Open", anotherDocPath).toDispatch();
			// 所有表格
	...
	public void setProp(String sName, String sValue) {
		Dispatch props = Dispatch.get(presentation, "CustomPresentationProperties").toDispatch();
	...
	public String getProp(String sName) {
		...
		try {
			Dispatch prop0 = Dispatch.call(presentation, "CustomPresentationProperties", sName).toDispatch();
	...
	public void fackChange() {
		Dispatch selection = Dispatch.call(presentation, "Range", 0, 0).toDispatch();
	...
	/**
	 * 保护当前档,如果不存在, 使用expression.Protect(Type, NoReset, Passpower_point)
	 * 
	 * @param pwd
	 * @param type
	 ...
	 * 
	 */
	public void protectedPowerPoint(String pwd, String type) {
		String protectionType = Dispatch.get(presentation, "ProtectionType").toString();
		if (protectionType.equals("-1")) {
			Dispatch.call(presentation, "Protect", Integer.parseInt(type), new Variant(true), pwd);
		}
	}
	 ...
	 * 
	 */
	public void unProtectedPowerPoint(String pwd) {
		String protectionType = Dispatch.get(presentation, "ProtectionType").toString();
		if (!protectionType.equals("0") && !protectionType.equals("-1")) {
			Dispatch.call(presentation, "Unprotect", pwd);
		}
	}
	...
	public String getProtectedType() {
		return Dispatch.get(presentation, "ProtectionType").toString();
	}
	...
	public void setAutomationSecurity(int value) {
		powerPoint.setProperty("AutomationSecurity", new Variant(value));
	}

	/**
	 * 在power_point中插入标签 labelName是标签名，labelValue是标签值
	 * 
	 * @param labelName
	 * @param labelValue
	 */
	public void insertLabelValue(String labelName, String labelValue) {
		...
		Dispatch bookMarks = Dispatch.call(presentation, "Bookmarks").toDispatch();
	...
	/**
	 * 在power_point中插入标签 labelName是标签名
	 * 
	 * @param labelName
	 */
